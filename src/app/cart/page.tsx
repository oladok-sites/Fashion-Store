...
export default function FavouritesClothes() {
	...
	const cartlClothes = cart.map((id) => clothes.find((item) => String(item.id) === id)).filter(Boolean);
				...
				{cartlClothes.map((clothe) => (
					<div key={clothe.id}>
						<div className="border border-black/40 mb-3.5 relative">
							...
							<button className="absolute bottom-2.5 left-0 right-0 mx-auto uppercase text-[12px] bg-[#d9d9d9] max-w-23.25 h-8.5 leading-8.5 px-5 hover:bg-[#d9d9d9]/80 hover:text-[#0c0c0c]/80 component-transition" onClick={() => setCartClothes(String(clothe.id))}>
								remove
							</button>
						</div>
