...
export default function HomeNewClient({ products }: { products: Product[] }) {
		...
		<section className="flex flex-col py-15 scroll-mt-37.5" id="new">
			...
			>
				{products.map((product, index) => (
					<>
						<SwiperSlide key={product.id} className="w-76.25">
							<div className="border border-black/40 mb-3.5 relative h-78.25">
								<Image src={product.images[0]} alt={product.title} fill className="object-cover object-center" loading='lazy' />
								<button className="w-8.5 h-8.5 text-[#0c0c0c] bg-[#dcdcdc] absolute bottom-0 left-0 right-0 mx-auto">
									+
								</button>
							</div>
